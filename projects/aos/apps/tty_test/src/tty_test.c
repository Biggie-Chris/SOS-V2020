...
/*
tty_test.c 是一个运行在 sos 操作系统之上的、无特权的用户进程。
它的核心任务是通过调用标准 C 库函数 (printf, read, write, malloc, mmap, sleep 等)，
间接地触发向 sos 的系统调用 IPC 请求
*/

/*主要是测试这几个：
正确处理无效输入: 当用户传来一个空指针或越界地址时，sos 应该返回一个错误码（如 -EFAULT），而不是自己崩溃。
正确处理边界条件: 当读写操作跨越页边界时，sos 的虚拟内存子系统应该能正确处理，而不是只在一个页内工作。
正确处理资源耗尽: 当用户请求大量内存时，sos 应该能优雅地处理内存不足的情况，而不是死机。
*/
